    ...
    */
    ...
    setFeatureState(source: string | { sourceId: string, sourceLayer: string }, feature: string, key: string, value: any) {
        ...
        if (sourceCache === undefined) {
            this.fire('error', {
                error: new Error(`The source '${sourceId}' does not exist in the map's style.`)
            });
            return;
        }
        ...
        if (sourceType === 'vector' && !sourceLayer) {
            this.fire('error', {
                error: new Error(`The sourceLayer parameter must be provided for vector source types.`)
            });
            return;
